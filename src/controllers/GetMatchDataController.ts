...
export const MatchData = async (req: Request, res: Response) => {
    if(!req.cookies.token || !req.cookies.entitlements || !req.cookies.puuid || !req.body.matchId) {
    ...
    const response = await getMatchData.handle(
        req.cookies.token, req.cookies.entitlements, req.cookies.puuid, req.body.matchId
    );
